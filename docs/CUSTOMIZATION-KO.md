...
`assets/css/app.css`의 CSS 변수 하나로 UI 전체가 따라옵니다.
...
앱 안의 Backup & storage → Display에서 `WebApp_House_Style.md` 3장의 6단계(6/8/10/12/14/17px, 기본 12px)로
직접 조절할 수 있습니다(`assets/js/app.js`의 `UI_SCALE_STEPS`/`setUiScale()`, `localStorage` 키
`petal.uiScale.v1`). 코드에서 기본값을 바꾸려면 `--ui-scale`만 고치면 되고, 이 노브는 책 읽기 화면의
글자 크기(Reading settings의 `Text size` 슬라이더, `assets/js/fonts.js`)와는 완전히 별개입니다.
