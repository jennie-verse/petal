...
## 2026-08-26 README 버전 표기 정정 (B-4)

- **문제**: 실제 버전은 1.4.4(`package.json`, `assets/js/backup.js`의 `APP_VERSION`, Service Worker 캐시 이름 전부 일치)인데 `README.md` 첫머리만 `1.4.2`로 남아 있었습니다.
- **원인**: `tests/deployment.test.mjs`의 "public deployment documentation matches the current release" 검사가 `현재 버전은 \`1.4.2\`` 를 **기대값으로 그대로 박아 두고 있었습니다.** 같은 테스트 안에서 `guide`/`customization` 문서는 이미 `1.4.4` 문자열을 기대하는데, `readme` 쪽만 옛 버전을 "정답"으로 고정하고 있었던 것 — 이 테스트 자체가 드리프트의 원인이었습니다. README를 1.4.4로 고치기만 하고 이 테스트를 그대로 두면 Actions가 실패하므로, 이번에 정규식을 `1.4.4`로 함께 고치고 `1.4.2`를 금지 패턴 목록에도 추가했습니다.
- **바꾼 파일**: `README.md`(5번째 줄), `tests/deployment.test.mjs`(68·73번째 줄). 코드(런타임 동작)는 변경 없음 — Service Worker·캐시 이름·`APP_VERSION` 전부 그대로.
- **버전 표기가 어긋난 다른 곳**: 없음. `docs/TEST-REPORT.md`의 "v1.4.2 설정 이름 정정"·"v1.4.1 자동 검증 보강"은 그 **당시 버전을 가리키는 날짜 기록**이라 정정 대상이 아닙니다(다른 앱들의 TEST-REPORT.md도 같은 방식으로 과거 버전을 그대로 남깁니다).
- **재발 방지 제안**: 버전 문자열을 여러 문서에 손으로 흩어 적지 말고, `package.json`의 `version`을 **단일 출처**로 삼아 README·GITHUB-PAGES-KO·CUSTOMIZATION 같은 문서가 그 값을 참조하도록(예: 빌드 시 치환하거나, 문서에는 숫자 대신 "package.json 참고"라고만 적기) 바꾸면, 문서 여러 곳에 같은 숫자를 손으로 맞추다 하나를 놓치는 이번 같은 일이 구조적으로 사라집니다. (제안만 함, 구현하지 않음)
- **검증**: `npm test` 22/22 통과, `npm run test:syntax` 통과.

## v1.4.2 설정 이름 정정

- 검증일: 2026-08-24
- 라이브러리 톱니 버튼이 실제로 여는 대상에 맞게 접근성 이름을 `Journal settings`로 정정했습니다.
- 배포·백업·Service Worker 버전을 1.4.2로 맞추고 회귀 검사에 버튼 이름과 이벤트 배선 계약을 추가했습니다.
...
- **Pass:** 22개 회귀 검사와 전체 syntax; content-off artifact, pending 정제, 자정 분할, exact/inferred 및 범위 redaction.
- **Pass:** desktop·390×844 Journal sheet, overflow 0, console warning/error 0.
- **Pending:** 실제 private E2E와 기존 Pending iOS/상용 EPUB 항목은 그대로 실기기 확인 필요.
