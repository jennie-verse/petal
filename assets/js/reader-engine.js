...
export class ReaderEngineAdapter extends EventTarget {
  ...
  async open(fileRecord, initialLocator, preferences, annotations = []) {
    ...
    const onLoad = event => this.#prepareDocument(event.detail);
  ...
  #prepareDocument({ doc, index }) {
    ...
    doc.addEventListener("pointerup", event => {
      ...
      const fraction = event.clientX / Math.max(1, doc.defaultView.innerWidth);
  ...
  setLayout(preferences = this.preferences) {
    ...
    const textWidthPixels = Math.round(
      (this.preferences.textWidth || 68) * (this.preferences.fontSize || DEFAULT_FONT_SIZE) * .52
    );
    renderer.setAttribute("max-inline-size", `${textWidthPixels}px`);
