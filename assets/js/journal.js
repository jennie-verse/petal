...
/* A session shorter than this is almost always an open-then-immediately-close
   tap, not real reading — skip queuing it to Daybook (the book's own local
   progress is still saved either way). */
const MIN_SESSION_SECONDS = 180;
...
export async function finishReadingSession(reason = "closed") {
  ...
  if (session.activeSeconds >= MIN_SESSION_SECONDS) await queueReadingSession(session, session.book);
}
