...
export function readingSessionRecords(session, book) {
  ...
  do {
    ...
    records.push({
      ...
      data: {
        ...
        ...(session.importedHistory ? { importedHistory: true } : {}),
      },
    });
    cursor = segmentEnd;
...
export function artifactToJournalRecord(storeName, record, book, operation = "created", options = {}) {
  ...
  return {
    ...
    title: String(book.title || "Untitled book"),
    data: { ...common, ...details, ...(options.importedHistory ? { importedHistory: true } : {}) },
  };
}
