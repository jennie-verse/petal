...
import {
  ...
  queueReadingSession, redactJournalContent, toggleJournal, isJournalContentEnabled, setJournalContentEnabled
} from "./journal.js";
...
// App-chrome text size (menus, lists, settings) — separate from the book
// reading font size above, which lives in state.preferences/IndexedDB and is
// per-book content. This one is a small device-local preference, so it uses
// localStorage like the equivalent control in every other app in the suite.
const UI_SCALE_STEPS = [6, 8, 10, 12, 14, 17];
const UI_SCALE_DEFAULT = 12;
const UI_SCALE_KEY = "petal.uiScale.v1";
function uiScaleStep() {
  const stored = Number(localStorage.getItem(UI_SCALE_KEY));
  return UI_SCALE_STEPS.includes(stored) ? stored : UI_SCALE_DEFAULT;
}
function applyUiScale() {
  document.documentElement.style.setProperty("--ui-scale", `${uiScaleStep()}px`);
}
function setUiScale(step) {
  if (!UI_SCALE_STEPS.includes(step)) return;
  localStorage.setItem(UI_SCALE_KEY, String(step));
  applyUiScale();
}
...
async function renderLibrary() {
  ...
  const header = `<header class="library-header shell-width">
    ...
    <div class="header-actions">
      <button class="icon-button" data-action="backup" aria-label="Settings">${icon("settings")}</button>
    </div>
  </header>`;
  if (!books.length) {
...
async function renderReader(bookId) {
  ...
  app.innerHTML = `<div class="app reader-screen">
    <header class="reader-toolbar" id="reader-toolbar">
      ...
      <nav class="reader-actions" aria-label="Reading tools">
        <button class="icon-button" data-action="view-menu" aria-label="View options">${icon("more")}</button>
      </nav>
    </header>
  ...
  state.reader.addEventListener("selection", event => {
    if (event.detail) showSelectionBar(event.detail);
    else clearSelection();
  });
...
function openSettings() {
  ...
  const body = `<div class="segmented">${presetNames.map(name => `<button data-action="preset" data-preset="${name}" class="${p.preset === name ? "selected" : ""}">${name === "large" ? "Large Print" : name[0].toUpperCase() + name.slice(1)}</button>`).join("")}</div>
    ...
    <div class="setting-row"><span>Page animation</span><button class="switch" data-action="setting-toggle" data-key="animation" role="switch" aria-checked="${p.animation}" aria-label="Page animation"></button></div>
    <div class="setting-row"><span>Keep publisher styles</span><button class="switch" data-action="setting-toggle" data-key="publisherStyles" role="switch" aria-checked="${p.publisherStyles}" aria-label="Keep publisher styles"></button></div>`;
...
function openViewMenu() {
  const body = `<div class="button-row" style="display:grid">
      <button class="button secondary" data-action="contents">${icon("menu")}Table of contents</button>
      <button class="button secondary" data-action="search">${icon("search")}Search book</button>
      <button class="button secondary" data-action="records">${icon("note")}Reading records</button>
      <button class="button secondary" data-action="settings"><span aria-hidden="true" style="font:20px Georgia">Aa</span>Reading settings</button>
    </div>`;
  overlay(sheetTemplate("View", body));
}
...
async function openRecords(tab = "highlights") {
  ...
  const body = `<div class="segmented tabs" role="tablist" aria-label="Reading records">${tabs.map(name => `<button data-action="record-tab" data-tab="${name}" class="${name === tab ? "selected" : ""}" role="tab" aria-selected="${name === tab}">${name[0].toUpperCase() + name.slice(1)}</button>`).join("")}</div>
    <div role="tabpanel">
...
// Petal has no device sync beyond the journal, so Token/Device name/Sync
// on-off (house style §9-1) live inside this one "Journal" section rather
// than a separate Sync section.
async function journalSectionHtml() {
  ...
  return `<section class="utility-section">
    <h2>Journal</h2>
    <div class="settings-group">
      ...
      <input id="journal-token" class="field-input" type="password" autocomplete="off" placeholder="${journal.hasToken ? "Token already stored" : "Paste token"}" aria-describedby="journal-privacy">
      ...
      <input id="journal-device" class="field-input" value="${escapeHtml(context.label)}" placeholder="iphone-home">
    ...
    <div class="settings-group">
      ...
      <div class="setting-row"><span><strong>Upload content to private Journal</strong><small>Controls quotes, notes, definitions, examples and sentences</small></span><button class="switch" data-action="journal-content-toggle" role="switch" aria-checked="${isJournalContentEnabled()}" aria-label="Upload Petal content"></button></div>
      <p class="dictionary-meta">This is separate from Daybook Compact. Turning it off sanitizes pending uploads but does not erase content already present in Git history.</p>
      <button class="button danger" data-action="journal-redact">Remove content from journal dates</button>
      <p class="dictionary-meta">Uses the date range below. The private repository keeps Git history, so old commits may still contain the previous text.</p>
    ...
    <div class="settings-group">
      ...
      <div class="journal-range"><label>From<input id="journal-from" class="field-input" type="date" value="${range.from}"></label><label>To<input id="journal-to" class="field-input" type="date" value="${range.to}"></label></div>
...
async function renderBackup() {
  ...
  const journalSection = await journalSectionHtml();
  app.innerHTML = `<div class="app screen backup-screen">
    <header class="utility-header"><button class="icon-button" data-action="library" aria-label="Back to library">${icon("back")}</button><h1>Settings</h1><span></span></header>
    <main id="main" class="utility-main">
      ...
      <section class="utility-section"><h2>Display</h2><p class="dictionary-meta">The app's own menus and lists — separate from the book text size in Reading settings.</p>
        <div class="button-row" role="group" aria-label="App text size">${UI_SCALE_STEPS.map(step => `<button class="button secondary${uiScaleStep() === step ? " selected" : ""}" data-action="ui-scale" data-step="${step}" aria-pressed="${uiScaleStep() === step}">${step}px</button>`).join("")}</div>
        <button class="text-button" data-action="ui-scale-reset">Reset</button>
      </section>
      ${journalSection}
      ...
      <section class="utility-section"><h2>Manage storage</h2><div class="storage-row"><span>Book copies</span><strong>${formatBytes(booksSize)}</strong></div>
        ...
        ${state.storage.persisted ? "" : `<button class="text-button" data-action="request-storage">${icon("shield")}Request storage protection</button>`}
      </section>
      ...
      <section class="utility-section"><h2>About</h2><p class="dictionary-meta">App version ${APP_BUILD}</p></section>
...
app.addEventListener("click", async event => {
  ...
  try {
    if (action === "import") epubInput.click();
    ...
    else if (action === "view-menu") openViewMenu();
    ...
    else if (action === "ui-scale") { setUiScale(Number(button.dataset.step)); await renderBackup(); }
    else if (action === "ui-scale-reset") { setUiScale(UI_SCALE_DEFAULT); await renderBackup(); }
    ...
    else if (action === "journal-content-toggle") {
      await setJournalContentEnabled(!isJournalContentEnabled());
      await renderBackup();
    }
    else if (action === "journal-redact") {
      if (!getJournalState().enabled) { toast("Turn on Include in journal first."); return; }
      if (isJournalContentEnabled()) { toast("Turn off Upload content on every Petal installation first."); return; }
      ...
      if (confirm(`Remove Petal quotes, notes, definitions, examples and sentences from ${from} through ${to}? This creates newer sanitized records; older Git history remains.`)) {
        const result = await redactJournalContent({ from, to });
      ...
      const first = confirm("Erase all Petal Reader data stored on this device? This cannot be undone. Export a JSON backup first if you want to keep your records.");
      if (first && confirm(`This permanently deletes ${state.books.length} book${state.books.length === 1 ? "" : "s"} and every highlight, note, bookmark and word you've saved. There is no undo. Erase everything?`)) {
