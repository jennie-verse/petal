class Sha256 {
  ...
  update(input) {
    ...
    while (offset + 64 <= merged.length) {
      this.block(merged.subarray(offset, offset + 64));
      offset += 64;
    }
  ...
  digest() {
    ...
    const padLength = this.buffer.length < 56 ? 64 : 128;
    const final = new Uint8Array(padLength);
    final.set(this.buffer);
    final[this.buffer.length] = 0x80;
    const view = new DataView(final.buffer);
    view.setUint32(final.length - 8, Math.floor(totalBits / 0x100000000));
    view.setUint32(final.length - 4, totalBits >>> 0);
    for (let offset = 0; offset < final.length; offset += 64) this.block(final.subarray(offset, offset + 64));
...
self.onmessage = async event => {
  ...
  for (let offset = 0; offset < file.size; offset += chunkSize) {
    ...
    sha.update(bytes);
    self.postMessage({ type: "progress", value: Math.min(1, (offset + bytes.length) / file.size) });
  }
  self.postMessage({ type: "done", hash: sha.digest() });
};
